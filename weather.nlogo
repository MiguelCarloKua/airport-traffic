...
to setup
  ...
  ; Get initial weather condition from the interface chooser
...
to set-weather-effects
  ...
  let current-weather (word weather-condition)

  ; Reset multiplier based on weather
  ifelse current-weather = "clear" [
    set weather-speed-multiplier 1.0
  ][
    ifelse current-weather = "rain" [
      set weather-speed-multiplier 0.8
    ][
      ifelse current-weather = "fog" [
        set weather-speed-multiplier 0.6
      ][
        ifelse current-weather = "snow" [
