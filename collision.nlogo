...
to go
  ...
  ask planes [
    ...
    if current-state = "ready" [
      ; Ready departing planes move towards their assigned gate
      ifelse member? patch-here gate-patches [
         ; Check if this is the *correct* assigned gate
        if patch-here = gate-assigned [
          set current-state "to-runway" ; Or whatever the next state should be
          ; Potentially wait or load here
        ]
        ; If it's a gate but not the assigned one, it should have been prevented.
      ] [
        taxi-to-gate ; Move towards the assigned gate
      ]
    ]
    if current-state = "to-runway" [
      ; Move towards runway for departure
       ifelse member? patch-here runway-patches [
        set current-state "departed"
        set color gray
      ] [
