; Airport Ground Traffic Simulation - NAIA Model
; CMPLXSY S19 Project
...
towers-own [
  controlled-planes runway-queue gate-assignments
]

trucks-own [
  truck-type assigned-plane home-base service-time busy
]
...
to setup-agents
  ...
  ; Create initial departing planes at hangars
  let num-initial-planes 3
  let created-planes 0
  while [created-planes < num-initial-planes] [
    ...
    if target-hangar != nobody [
      ...
      set created-planes created-planes + 1
    ]
  ]
end
...
to manage-traffic
  ...
  ; --- Departure Logic ---
  ; Find planes ready to go to runway
  let ready-to-depart planes with [current-state = "to-runway"]
  ask ready-to-depart [
    ; Check if already on runway
    ifelse member? patch-here runway-patches [
      set current-state "departed"
      set color gray
      ; Gate is now free - Clear gate assignment to free it up for others
      let my-gate gate-assigned
      if my-gate != nobody [
        ask my-gate [ set plabel "" ] ; Optional: Clear gate label
      ]
      set gate-assigned nobody
      set destination nobody
    ][
      ; Move towards runway if not on it
      taxi-to-runway
    ]
  ]

  ; Find planes ready to depart (old state name for transition)
  let ready-for-runway planes with [current-state = "awaiting-takeoff"]
  if any? ready-for-runway [
    ask one-of ready-for-runway [
      set current-state "to-runway" ; Transition to new state name
    ]
  ]
end
...
; --- Refined taxi-to-runway with collision check ---
to taxi-to-runway
  let options neighbors4 with [member? self navigable-patches]
  if any? options [
    let target-runway destination ; Use the plane's destination (runway)
    if target-runway = nobody [
      set target-runway min-one-of runway-patches [distance myself]
    ]
    if target-runway != nobody [
...
; --- Refined taxi-to-gate with collision check ---
to taxi-to-gate
  let options neighbors4 with [member? self navigable-patches]
  if any? options [
    ...
    if target-gate = nobody [
      ; Fallback if gate not assigned yet, find closest gate
      set target-gate min-one-of gate-patches [distance myself]
    ]
    if target-gate != nobody [
...
to respawn-plane
  ...
  set base-speed 0.5
  set wait-counter 0
  set gate-assigned nobody ; Clear gate assignment
  set destination nobody
  color-by-state
end
...
to color-by-state
  ...
  if current-state = "at-gate-ready" [ set color violet ]
end
...
; --- Keep the taxi-randomly procedure for any planes not using state-based movement ---
; But modify it to prevent collisions
to taxi-randomly
