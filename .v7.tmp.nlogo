...
to setup-fuelers
  ...
  ;let fueler-home-spot one-of fueler-home-patches
  create-fuelers set-fuelers [
    let spot one-of fueler-home-patches with [ not any? fuelers-here ]
    if spot = nobody [ set spot one-of fueler-home-patches ] ;; fallback
    move-to spot
    set home-patch spot
    set shape "truck"
    set color orange
...
to setup-airport-layout
  ...
  set fueler-home-patches patches with [pxcor >= 40 and pxcor <= 60 and pycor = 64]
  ask fueler-home-patches [set pcolor orange - 3]
  ...
  foreach sort gate-patches [
    ...
    let gx [pxcor] of g
    let gy [pycor] of g
    let b patch (gx - 1) gy  ;; boarding patch to the left
    let f patch (gx + 1) gy  ;; fueling patch to the right
...
to go
  ...
  ask customers [
    ...
    if purpose = "board" [
      ;; Find a plane that is in the "boarding" state
      let boarding-planes planes with [current-state = "boarding"]
      if any? boarding-planes [
        let nearest-boarding-plane min-one-of boarding-planes [ distance myself ]
        ;; The boarding-lane patch is to the left of the gate patch
        let gate-patch [gate-assigned] of nearest-boarding-plane
        if gate-patch != nobody [
          let gx [pxcor] of gate-patch
          let gy [pycor] of gate-patch
          let boarding-pad patch (gx - 1) gy
          ;; Only assign a destination if the plane still needs passengers
          if [passengers] of nearest-boarding-plane < [max-passengers] of nearest-boarding-plane [
            set destination-gate boarding-pad
          ]
        ]
      ]
    ...
    if destination-gate != nobody [
      ...
      ifelse distance destination-gate > 0.5 [
        ...
        if (purpose = "board") and [boarding-lane?] of destination-gate [
          let gate-patch patch ([pxcor] of destination-gate + 1) ([pycor] of destination-gate)
          let plane-here [gate-occupied-by] of gate-patch
          if (plane-here != nobody) and ([current-state] of plane-here = "boarding") and ([passengers] of plane-here < [max-passengers] of plane-here) [
  ...
  ask planes [
    ...
    if current-state = "fueling" [
      ...
      if fueling-remaining <= 0 [
        ...
        assign-passengers-to-plane self
      ]
    ]
...
to dispatch-fuelers
  ;; planes needing fuel and not already assigned to a fueler
  ...
  ask fuelers with [state = "idle"] [
    if any? waiting-planes [
      ...
      if p != nobody [
        ...
        set state "to-plane"
        ask p [ set fueler-assigned? true ]  ;; mark as taken immediately
        set waiting-planes waiting-planes with [self != p]
      ]
    ]
  ]
end
  ...
  if terminal-boarders < terminal-capacity [
    ...
    if n > 0 [
      create-customers n [
        ...
        set color yellow
        set purpose "board"
        set destination-gate nobody ;; Passengers are created without a destination
      ]
    ]
  ]
end
...
; passengers boarding assignement procedure
to assign-passengers-to-plane [pln]
  ;; how many more passengers are needed
  let needed max (list 0 (50 - [passengers] of pln))
  if needed > 0 [
    ;; pick only that many waiting passengers in terminal
    let terminal-waiters customers with [
      purpose = "board" and
      destination-gate = nobody and
      [pcolor] of patch-here = black
    ]
    let chosen n-of needed terminal-waiters
    let gate-pad one-of boarding-patches with [
      pxcor = [pxcor] of [gate-assigned] of pln and
      pycor = ([pycor] of [gate-assigned] of pln) - 1
    ]
    ask chosen [
      set destination-gate gate-pad
    ]
  ]
end
@#$#@#$#@
...
481
705
Customers in Terminal, Going out
passengers-walking
17
1
11
